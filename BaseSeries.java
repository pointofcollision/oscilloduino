...
public abstract class BaseSeries<E extends DataPointInterface> implements Series<E> {
    ...
    static private DataPoint dPoint=new DataPoint(1,1);
    /**
    ...
    public void appendData(E dataPoint, boolean scrollToEnd, int maxDataPoints, boolean silent) {
        ...
        if (!mData.isEmpty() && dataPoint.getX() < mData.get(mData.size()-1).getX()) {
           // throw new IllegalArgumentException("new x-value must be greater then the last value. x-values has to be ordered in ASC.");
            //we need to change this part to add in a new point. at the specified value.
            //this will never be called for the other appends, just the data vs dac plot
            //lets assume we will have a static array of 1000 data points, that we just change the
            //values of. this means we just update the values at each of the positions. Not the best idea.
            Log.d("message","x value looking at: "+String.valueOf(dataPoint.getX()));
               // Log.d("message",String.valueOf(i));
               //just round the value of the x to a place in 1000 digits
            double i=dataPoint.getX();
            double workableAnswer=1000.0*(i/5.0);
            int index=(int)workableAnswer;
            Log.d("message", "index looking at: " + String.valueOf(index));
            if (index <= 1000 && index>=0) {


                mData.remove(index);
                mData.add(index, dataPoint);
                Log.d("message", "y value looking at: " + String.valueOf(mData.get(index)));
